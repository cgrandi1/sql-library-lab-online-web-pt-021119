...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(characters.name) FROM character_books JOIN characters on characters.id = character_books.character_id JOIN books on books.id = character_books.book_id GROUP BY (characters.name) ORDER BY (characters.name) DESC"
end
